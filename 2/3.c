        ...
        if (isalnum(text_line[i])) {
          ...
          if (in_word) {
            ...
            if (sub != NULL && occurrence_len_for_this_seq < MAX_OCCURRENCES) {
              // Che oscenita' con un puntatore si evitava che la struct venisse
              // copiata in una variabile temporanea e quindi avrei potuto
              // evitare di ripetere 53 * 3 caratteri
