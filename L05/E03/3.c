...
line* read_file(char* filename, int* lines_len_, FILE** fp_)
{
    ...
    line* lines = calloc(lines_len, sizeof(line));
    for (int i = 0; i < lines_len; ++i) {
...
int main()
{
    ...
    FILE* fp;
    int lines_len;
    line* lines = read_file("corse.txt", &lines_len, &fp);
    if (lines == NULL) {
        return 1;
    }
    ...
    while (main_loop_flag) {
        ...
        case READ_FILE: {
            ...
            printf("lines_len: %d\n", lines_len);
            lines = read_file(filename, &lines_len, &fp);
            if (lines == NULL) {
                return 1;
            }

            is_sorted_datetime = false;
            sorted_lines_datetime = calloc(sizeof(line*), lines_len);
            for (int i = 0; i < lines_len; ++i) {
                sorted_lines_datetime[i] = &lines[i];
            }

            is_sorted_code = false;
            sorted_lines_code = calloc(sizeof(line*), lines_len);
            for (int i = 0; i < lines_len; ++i) {
                sorted_lines_code[i] = &lines[i];
            }

            is_sorted_start = false;
            sorted_lines_start = calloc(sizeof(line*), lines_len);
            for (int i = 0; i < lines_len; ++i) {
                sorted_lines_start[i] = &lines[i];
            }

            is_sorted_destination = false;
            sorted_lines_destination = calloc(sizeof(line*), lines_len);
            for (int i = 0; i < lines_len; ++i) {
                sorted_lines_destination[i] = &lines[i];
            }
